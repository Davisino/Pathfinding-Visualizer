...
// buttons
document.getElementById('vizualize-algorithm').addEventListener('click', vizualizeAlgoSelected)

document.getElementById('random-target').addEventListener('click', colorStartAndEnd);
document.getElementById('dijkstra-btn').addEventListener('click', function () {
   clearAlgorithmsList();
   algorithms['dijkstras'] = true;
   document.getElementById('vizualize-algorithm').innerHTML = `Vizualize Dijkstras`
});
document.getElementById('clear-grid').addEventListener('click', restoreToDefault);
document.getElementById('depth-firstSearch-btn').addEventListener('click', function () {
   clearAlgorithmsList();
   algorithms['dfs'] = true;
...
document.getElementById('breadth-firstSearch-btn').addEventListener('click', function () {
   clearAlgorithmsList();
   algorithms['bfs'] = true;
...
document.getElementById('algorithm-button').addEventListener('click', displayListOfAlgos);
...
function clearGrid() {
   ...
   let a = '13-14';
   document.getElementById(a).style.backgroundColor = 'rgb(28, 141, 132)';
}
...
function colorStartAndEnd(ifStart) {
   ...
   for (let i = 0; i < boxes.length; i++) {
      if (typeof boxes[i] !== 'number') {
         boxes[i].className = 'unvisited'
         boxes[i].style.borderColor = 'rgb(115, 160, 126)';
      }
   }
   ...
   document.getElementById(getNewStartAndTarget[0]).className = 'start';
   document.getElementById(getNewStartAndTarget[1]).className = 'target';
}
// Dijkstras
...
function useDepthFirstTraversal() {
   ...
   const animations = depthFirstTraversal(tableGraph.getVertexByValue(startValue));
   const shortestPathAnimation = animations.concat(getShortestPathDFS(animations));
   ...
   for (let idx = 0; idx < shortestPathAnimation.length; idx++) {
      ...
      if (shortestPathAnimation[idx][0] == '$') {
         ...
         setTimeout(() => {
            ...
            document.getElementById(`${getDataFromVertex}`).style.borderColor = 'white';
         }, idx * velocity)
      }
